...
export default async function AdminPage() {
  ...
  return (
    <main style={{ minHeight: "100vh", padding: "28px 32px" }}>
      <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, paddingBottom: 20, borderBottom: "1px solid var(--line)" }}>
        <div>
          <div style={{ marginBottom: 10 }}>
            <Logo height={30} />
          </div>
          <div className="mono" style={{ fontSize: 11, letterSpacing: "0.18em", color: "var(--ink-faint)", marginBottom: 6 }}>
            ADMIN
          </div>
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div style={{ background: "var(--bg-card)", border: "1px solid var(--line)", borderRadius: 10, padding: "16px 22px", minWidth: 140 }}>
      <div className="mono" style={{ fontSize: 28, color: "var(--accent)", fontWeight: 500 }}>{value}</div>
      <div style={{ fontSize: 12, color: "var(--ink-dim)", marginTop: 4 }}>{label}</div>
    </div>
  );
}